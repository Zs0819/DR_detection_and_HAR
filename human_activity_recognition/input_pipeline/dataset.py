...
from input_pipeline.create_tfrecords import train_record_file, test_record_file, val_record_file, create_tfrecords
from input_pipeline.preprocessing import preprocess



@gin.configurable
def load(existed_tfrecords, name, data_dir):

    window_size, shift_window_size = create_tfrecords(existed_tfrecords)
    ...
    if name == "hapt":
        ...
        ds_train = ds_train.window(size=window_size, shift=shift_window_size, drop_remainder=True)
        ...
        ds_val = ds_val.window(size=window_size, shift=shift_window_size, drop_remainder=True)
        ds_val = ds_val.flat_map(lambda data, label: tf.data.Dataset.zip((data, label))).batch(window_size, drop_remainder=True)

        ds_test = ds_test.map(parse_record).map(decode_record)
        ds_test = ds_test.window(size=window_size, shift=shift_window_size, drop_remainder=True)
