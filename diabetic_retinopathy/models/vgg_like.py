...
def vgg_like(input_shape, n_classes, base_filters, n_blocks, dense_units, dropout_rate):
    ...
    out = tf.keras.layers.GlobalAveragePooling2D(name='avg')(out)
    out = tf.keras.layers.Dense(dense_units, activation=tf.nn.relu, name='dense1')(out)
    out = tf.keras.layers.Dropout(dropout_rate, name='dropout1')(out)
    outputs = tf.keras.layers.Dense(n_classes, name='dense2')(out)

    return tf.keras.Model(inputs=inputs, outputs=outputs, name='vgg_like')


if __name__ == "__main__":
    model = vgg_like(input_shape=(512, 512, 3), n_classes=2, base_filters=8, n_blocks=4, dense_units=64, dropout_rate=0.5)
    model.summary()